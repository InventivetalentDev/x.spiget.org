(function(c){"function"===typeof define&&define.amd?define(["jquery"],c):"object"===typeof exports&&"function"===typeof require?c(require("jquery")):c(jQuery)})(function(c){function g(a,b){var d=c.noop,d={ajaxSettings:{},autoSelectFirst:!1,appendTo:document.body,serviceUrl:null,lookup:null,onSelect:null,width:"auto",minChars:1,maxHeight:300,deferRequestBy:0,params:{},formatResult:g.formatResult,delimiter:null,zIndex:9999,type:"GET",noCache:!1,onSearchStart:d,onSearchComplete:d,onSearchError:d,preserveInput:!1,
...
document.createElement("div");b.className=a;b.style.position="absolute";b.style.display="none";return b}}}();g.utils=k;c.Autocomplete=g;g.formatResult=function(a,b){if(!b)return a.value;var d="("+k.escapeRegExChars(b)+")";return a.value.replace(new RegExp(d,"gi"),"<strong>$1</strong>").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/&lt;(\/?strong)&gt;/g,"<$1>")};g.prototype={killerFn:null,initialize:function(){var a=this,b="."+a.classes.suggestion,
d=a.classes.selected,e=a.options,f;a.element.setAttribute("autocomplete","off");a.killerFn=function(b){c(b.target).closest("."+a.options.containerClass).length||(a.killSuggestions(),a.disableKillerFn())};a.noSuggestionsContainer=c('<div class="autocomplete-no-suggestion"></div>').html(this.options.noSuggestionNotice).get(0);a.suggestionsContainer=g.utils.createNode(e.containerClass);f=c(a.suggestionsContainer);f.appendTo(e.appendTo);"auto"!==e.width&&f.css("width",e.width);f.on("mouseover.autocomplete",
b,function(){a.activate(c(this).data("index"))});f.on("mouseout.autocomplete",function(){a.selectedIndex=-1;f.children("."+d).removeClass(d)});f.on("click.autocomplete",b,function(){a.select(c(this).data("index"));return!1});a.fixPositionCapture=function(){a.visible&&a.fixPosition()};c(window).on("resize.autocomplete",a.fixPositionCapture);a.el.on("keydown.autocomplete",function(b){a.onKeyPress(b)});a.el.on("keyup.autocomplete",function(b){a.onKeyUp(b)});a.el.on("blur.autocomplete",function(){a.onBlur()});
...
c(this.suggestionsContainer),b=a.parent().get(0);if(b===document.body||this.options.forceFixPosition){var d=this.options.orientation,e=a.outerHeight(),f=this.el.outerHeight(),l=this.el.offset(),h={top:l.top,left:l.left};if("auto"===d)var d=c(window).height(),m=c(window).scrollTop(),g=-m+l.top-e,d=Math.max(g,m+d-(l.top+f+e))===g?"top":"bottom";h.top="top"===d?h.top+-e:h.top+f;b!==document.body&&(b=a.css("opacity"),this.visible||a.css("opacity",0).show(),e=a.offsetParent().offset(),h.top-=e.top,h.left-=
e.left,this.visible||a.css("opacity",b).hide());"auto"===this.options.width&&(h.width=this.el.outerWidth()+"px");a.css(h)}},enableKillerFn:function(){c(document).on("click.autocomplete",this.killerFn)},disableKillerFn:function(){c(document).off("click.autocomplete",this.killerFn)},killSuggestions:function(){var a=this;a.stopKillSuggestions();a.intervalId=window.setInterval(function(){a.visible&&(a.options.preserveInput||a.el.val(a.currentValue),a.hide());a.stopKillSuggestions()},50)},stopKillSuggestions:function(){window.clearInterval(this.intervalId)},
isCursorAtEnd:function(){var a=this.el.val().length,b=this.element.selectionStart;return"number"===typeof b?b===a:document.selection?(b=document.selection.createRange(),b.moveStart("character",-a),a===b.text.length):!0},onKeyPress:function(a){if(!this.disabled&&!this.visible&&40===a.which&&this.currentValue)this.suggest();else if(!this.disabled&&this.visible){switch(a.which){case 27:this.el.val(this.currentValue);this.hide();break;case 39:if(this.hint&&this.options.onHint&&this.isCursorAtEnd()){this.selectHint();
break}return;case 9:if(this.hint&&this.options.onHint){this.selectHint();return}if(-1===this.selectedIndex){this.hide();return}this.select(this.selectedIndex);if(!1===this.options.tabDisabled)return;break;case 13:if(-1===this.selectedIndex){this.hide();return}this.select(this.selectedIndex);break;case 38:this.moveUp();break;case 40:this.moveDown();break;default:return}a.stopImmediatePropagation();a.preventDefault()}},onKeyUp:function(a){var b=this;if(!b.disabled){switch(a.which){case 38:case 40:return}clearInterval(b.onChangeInterval);
...
for(var b=this.badQueries,d=b.length;d--;)if(0===a.indexOf(b[d]))return!0;return!1},hide:function(){var a=c(this.suggestionsContainer);c.isFunction(this.options.onHide)&&this.visible&&this.options.onHide.call(this.element,a);this.visible=!1;this.selectedIndex=-1;clearInterval(this.onChangeInterval);c(this.suggestionsContainer).hide();this.signalHint(null)},suggest:function(){if(this.suggestions.length){var a=this.options,b=a.groupBy,d=a.formatResult,e=this.getQuery(this.currentValue),f=this.classes.suggestion,
g=this.classes.selected,h=c(this.suggestionsContainer),m=c(this.noSuggestionsContainer),k=a.beforeRender,n="",p;a.triggerSelectOnValidInput&&this.isExactMatch(e)?this.select(0):(c.each(this.suggestions,function(a,c){if(b){var h=n,g;g=c.data[b];p===g?g="":(p=g,g='<div class="autocomplete-group"><strong>'+p+"</strong></div>");n=h+g}n+='<div class="'+f+'" data-index="'+a+'">'+d(c,e,a)+"</div>"}),this.adjustContainerWidth(),m.detach(),h.html(n),c.isFunction(k)&&k.call(this.element,h,this.suggestions),
this.fixPosition(),h.show(),a.autoSelectFirst&&(this.selectedIndex=0,h.scrollTop(0),h.children("."+f).first().addClass(g)),this.visible=!0,this.findBestHint())}else this.options.showNoSuggestionNotice?this.noSuggestions():this.hide()},noSuggestions:function(){var a=c(this.suggestionsContainer),b=c(this.noSuggestionsContainer);this.adjustContainerWidth();b.detach();a.empty();a.append(b);this.fixPosition();a.show();this.visible=!0},adjustContainerWidth:function(){var a=this.options,b=c(this.suggestionsContainer);
"auto"===a.width&&(a=this.el.outerWidth(),b.css("width",0<a?a:300))},findBestHint:function(){var a=this.el.val().toLowerCase(),b=null;a&&(c.each(this.suggestions,function(d,c){var f=0===c.value.toLowerCase().indexOf(a);f&&(b=c);return!f}),this.signalHint(b))},signalHint:function(a){var b="";a&&(b=this.currentValue+a.value.substr(this.currentValue.length));this.hintValue!==b&&(this.hintValue=b,this.hint=a,(this.options.onHint||c.noop)(b))},verifySuggestionsFormat:function(a){return a.length&&"string"===
...
b=c.split(b);return 1===b.length?a:c.substr(0,c.length-b[b.length-1].length)+a},dispose:function(){this.el.off(".autocomplete").removeData("autocomplete");this.disableKillerFn();c(window).off("resize.autocomplete",this.fixPositionCapture);c(this.suggestionsContainer).remove()}};c.fn.autocomplete=c.fn.devbridgeAutocomplete=function(a,b){return arguments.length?this.each(function(){var d=c(this),e=d.data("autocomplete");if("string"===typeof a){if(e&&"function"===typeof e[a])e[a](b)}else e&&e.dispose&&
e.dispose(),e=new g(this,a),d.data("autocomplete",e)}):this.first().data("autocomplete")}});
